...
void SHA2::hash32(uint16_t bit_length, uint32_t *hash_constants) {
    ...
    while ((raw.length() % 512) != 448) {
        raw += "0";
    }

    // Append the length of the original message as a 64-bit binary string
    std::string lengthBits = std::bitset<64>(inputLength).to_string();
    raw += lengthBits;

    // Initialize chunks
    std::vector<uint32_t*> chunks;

    for (size_t i = 0; i < raw.length(); i += 512) {
        uint32_t *chunk = new uint32_t[64]();
        for (size_t j = 0; j < 16; j++) {
            chunk[j] = std::stoul(raw.substr(i + j * 32, 32), nullptr, 2);
        }
    ...
    for (int i = 0; i < 8; i++) {
        ...
        ss << std::hex << std::setw(8) << std::setfill('0') << hash_constants[i] ;
        this->hash += ss.str();
    }
};
...
void SHA2::hash64(uint16_t bit_length, uint64_t *hash_constants) {
    ...
    while ((raw.length() % 1024) != 896) {
        raw += "0";
    }

    // Append the length of the original message as a 128-bit binary string
    std::string lengthBits = std::bitset<128>(inputLength).to_string();
    raw += lengthBits;

    // Initialize chunks
    std::vector<uint64_t*> chunks;

    for (size_t i = 0; i < raw.length(); i += 1024) {
        uint64_t *chunk = new uint64_t[80]();
        for (size_t j = 0; j < 16; j++) {
            chunk[j] = std::stoull(raw.substr(i + j * 64, 64), nullptr, 2);
        }
        chunks.push_back(chunk);
    }
